...
(defn header [component title]
  (let [{:keys [db/id ui/new-item-text]} (prim/props component)]
    (dom/header :.header
      (dom/h1 title)
      (dom/input :.new-todo {:value       (or new-item-text "")
                             :onKeyDown   (fn [evt]
                                            (when (is-enter? evt)
...
(defn filter-footer [component num-todos num-completed]
  (let [{:keys [db/id list/filter]} (prim/props component)
    ...
    (dom/footer :.footer
      ...
      (dom/ul :.filters
        (dom/li (dom/a {:className (when (or (nil? filter) (= :list.filter/none filter)) "selected")
                        :href      "#"} (tr "All")))
        (dom/li (dom/a {:className (when (= :list.filter/active filter) "selected")
                        :href      "#/active"} (tr "Active")))
        (dom/li (dom/a {:className (when (= :list.filter/completed filter) "selected")
                        :href      "#/completed"} (tr "Completed"))))
      (when (pos? num-completed)
        (dom/button :.clear-completed {:onClick #(prim/transact! component `[(api/todo-clear-complete {:list-id ~id})])} (tr "Clear Completed"))))))
